...
const buildTableRow = async (
  ...
  if (isVNode(vNode) && vNode.properties) {
    ...
    const firstChildNode = isVNode(firstChild) ? firstChild : undefined
    if (
      vNode.properties.style?.height ||
      (firstChildNode &&
        firstChildNode.properties?.style &&
        firstChildNode.properties.style.height)
    ) {
      const heightValue =
        vNode.properties.style?.height ||
        (firstChildNode &&
  ...
  if (vNodeHasChildren(vNode)) {
    const tableColumns = (vNode.children || []).filter((childVNode): childVNode is VNode =>
      isVNode(childVNode) && TABLE_CELL_TAGS.has(childVNode.tagName || '')
    )
...
const buildTableGrid = (vNode: VNode, attributes: TableAttributes): XMLBuilderType => {
  ...
  if (vNodeHasChildren(vNode)) {
    const gridColumns = (vNode.children || []).filter(
      (childVNode): childVNode is VNode => isVNode(childVNode) && childVNode.tagName === 'col'
    )
