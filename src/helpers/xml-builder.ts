...
const buildTableRow = async (
  ...
  if (vNodeHasChildren(vNode)) {
    const tableColumns = (vNode.children || []).filter((childVNode) => isVNode(childVNode) && TABLE_CELL_TAGS.has(childVNode.tagName || ''))
...
const buildTableGrid = (vNode: VNode, attributes: TableAttributes): XMLBuilderType => {
  ...
  if (vNodeHasChildren(vNode)) {
    const gridColumns = (vNode.children || []).filter((childVNode) => isVNode(childVNode) && childVNode.tagName === 'col')
