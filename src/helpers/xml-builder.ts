...
const buildTextRunFragment = (
  ...
  // Bolt: Avoid deep cloning flat RunAttributes. buildRunProperties doesn't mutate attributes.
...
const buildRun = async (
  ...
  const runFragment = fragment({ namespaceAlias: { w: namespaces.w } }).ele('@w', 'r')
  // Bolt: Avoid deep cloning flat ParagraphAttributes/RunAttributes. buildRunProperties doesn't mutate attributes.
  const runPropertiesFragment = buildRunProperties(attributes)
  ...
  if (isVNode(vNode) && RUN_TAGS.has((vNode as VNodeType).tagName || '')) {
    ...
    // Bolt: Use shallow object spread operation ({ ...attributes }) instead of cloneDeep for flat objects
    // This is significantly faster and allocates less memory.
