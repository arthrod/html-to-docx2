...
import {
	reportUnmappedType,
	type UnmappedTypeHandling,
} from "./unmapped-type-reporter";
			...
			if (isVText(tempVNode)) {
				...
				if (
					...
					if (Array.isArray(spanFragment)) {
						const flatSpanFragments = spanFragment.flat(
		...
		if (response && docxDocumentInstance) {
			docxDocumentInstance.zip
				...
				.file(
					...
					base64ToUint8Array(response.fileContent),
					{
						createFolders: false,
					},
				);
...
const buildHorizontalAlignment = (
	...
	const alignment =
		horizontalAlignment === "justify" ? "both" : horizontalAlignment;
	return fragment({ namespaceAlias: { w: namespaces.w } })
		.ele("@w", "jc")
		.att("@w", "val", alignment)
		.up();
...
const processImageSource = async (
	...
	const imageProperties = getImageDimensions(
		base64ToUint8Array(normalizedBase64),
	);
		...
		if (!tblGridEmitted) {
			for (const childVNode of (vNode.children || []) as VNodeType[]) {
				...
				if (childVNode.tagName === "tr") {
					...
					childVNode.tagName === "thead" ||
					childVNode.tagName === "tbody"
				) {
