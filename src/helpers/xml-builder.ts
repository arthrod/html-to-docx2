...
const buildRun = async (
  ...
  if (
    ...
    while (vNodes.length) {
      ...
      const tempVn = tempVNode
      if (isVNode(tempVn) && tempVn.children?.length) {
...
const buildTableRow = async (
  ...
  if (vNodeHasChildren(vNode)) {
    const tableColumns = (vNode.children || []).filter(isVNode).filter((childVNode) => TABLE_CELL_TAGS.has(childVNode.tagName || ''))
...
const buildTableGrid = (vNode: VNode, attributes: TableAttributes): XMLBuilderType => {
  ...
  if (vNodeHasChildren(vNode)) {
    const gridColumns = (vNode.children || []).filter(isVNode).filter((childVNode) => childVNode.tagName === 'col')
...
const buildTableGridFromTableRow = (
  ...
  if (vNodeHasChildren(vNode)) {
    ...
    for (const childVNode of (vNode.children || []).filter(isVNode)) {
    ...
    if (!tblGridEmitted) {
      for (const childVNode of (vNode.children || []).filter(isVNode)) {
        ...
        if (childVNode.tagName === 'tr') {
          ...
          for (const grandChildVNode of (childVNode.children || []).filter(isVNode)) {
