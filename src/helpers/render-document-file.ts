...
const asVNode = (node: VTreeElement): VNode | null => {
...
const asVText = (node: VTreeElement): VText | null => {
  if (typeof node !== 'object' || node === null || Array.isArray(node)) {
    return null
  }
...
const isInlineElement = (node: VTreeElement): boolean =>
...
const containsSpecialElements = (node: VTreeElement): boolean => {
...
const serializeVNodeToSVG = (node: VTreeElement, isRoot = false): string => {
...
async function findXMLEquivalent(
  ...
  if (vNode.tagName === 'div' && vNodeHasChildren(vNode)) {
    ...
    if (hasSpecialChildren) {
      ...
      let currentInlineGroup: VTreeElement[] = []
