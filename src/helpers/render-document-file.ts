...
type VTree = VNode | VText | (VNode | VText)[] | null
...
const asVNode = (node: VNode | VText | null): VNode | null => {
...
const asVText = (node: VNode | VText | null): VText | null => {
  if (typeof node !== 'object' || node === null || Array.isArray(node)) {
    return null
  }
  return typeof node.text === 'string' ? node : null
}

// Check if a vNode is an inline element
const isInlineElement = (node: VNode | VText | null): boolean =>
...
const containsSpecialElements = (node: VNode | VText | null): boolean => {
...
const serializeVNodeToSVG = (node: VNode | VText | null, isRoot = false): string => {
    ...
    if (
      ...
      for (const childVNode of tempNode.children) {
        ...
        if (['ul', 'ol'].includes(childNode.tagName || '')) {
          ...
          (
            tempVNodeObjects[tempVNodeObjects.length - 1].node.tagName || ''
          ).toLowerCase() === 'p' &&
  ...
  switch (vNode.tagName) {
    ...
    case 'figure':
      if (vNodeHasChildren(vNode)) {
        ...
        const processImageInNode = async (node: VNode | VText | null): Promise<void> => {
