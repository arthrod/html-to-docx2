...
const serializeVNodeToSVG = (node: VNodeType | VTextType, isRoot = false): string => {
  ...
  if (textNode) {
    // ⚡ Bolt: Use optimized single-pass escapeXml utility instead of chained regex .replace()
    // This avoids intermediate string allocations and regex compilations in a hot path.
