...
class DocxDocument {
  ...
  commentByNumericIdMap: Map<number, StoredComment>
  ...
  constructor(properties: DocxDocumentProperties) {
    ...
    this.commentByNumericIdMap = new Map()
  ...
  ensureComment(data: Partial<CommentPayload>, parentParaId?: string): number {
    ...
    const existing = this.commentByNumericIdMap.get(numericId)
    if (existing) {
    ...
    this.commentByNumericIdMap.set(numericId, entry)
