...
export function normalizeEncodingLabel(label: unknown) {
...
function normalizeMetaDeclaredEncoding(label: unknown) {
...
function extractCharsetFromContent(contentBytes: Uint8Array | null) {
...
function prescanForMetaCharset(data: Uint8Array) {
  ...
  while (i < n && i < maxTotalScan && nonComment < maxNonComment) {
    ...
    while (k < n && k < maxTotalScan) {
      ...
      while (k < n) {
        const c = data[k]
        if (ASCII_WHITESPACE.has(c) || c === 0x3d || c === 0x3e || c === 0x2f || c === 0x3c)
          break
        k += 1
      }
      const attrEnd = k
...
export interface HTMLEncodingOptions {
  transportEncoding?: unknown;
}

export function sniffHTMLEncoding(data: Uint8Array, { transportEncoding = null }: HTMLEncodingOptions = {}) {
...
export function decodeHTML(data: Uint8Array, { transportEncoding = null }: HTMLEncodingOptions = {}) {
