...
function createMinimalJPEG(width = 64, height = 64) {
  ...
  dv.setUint8(19, 0) // padding to match 16-byte length (which includes the 2 bytes of length itself, so 14 bytes after length: 6+4+1+1+1+2+2+1+1 = 19 - but wait, offset 4 to 19 is 16 bytes. Offset 4,5 are length. Offset 6-19 are 14 bytes. So 4 + 16 = 20. Total length is 20)
