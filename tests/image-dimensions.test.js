...
// Minimal valid lossy WebP (32x32) - VP8 format
function createMinimalLossyWebP(width = 32, height = 32) {
  const data = new Uint8Array(40)
  const dv = new DataView(data.buffer)
  ...
  // VP8 frame header
  dv.setUint8(20, 0) // keyframe bit + version + show_frame
  dv.setUint8(21, 0)
  dv.setUint8(22, 0) // 3 bytes for frame size
  dv.setUint8(23, 0x9d) // Sync code byte 1
  dv.setUint8(24, 0x01) // Sync code byte 2
  dv.setUint8(25, 0x2a) // Sync code byte 3
...
describe('Image Dimensions', () => {
  describe('getImageDimensions', () => {
    ...
    it('should parse lossy WebP dimensions (32x32)', () => {
      const result = getImageDimensions(createMinimalLossyWebP(32, 32))
      expect(result).toEqual({ width: 32, height: 32, type: 'webp' })
    })

    it('should handle malformed WebP with missing format chunk', () => {
      const data = new Uint8Array([
        0x52, 0x49, 0x46, 0x46, 0, 0, 0, 0, 0x57, 0x45, 0x42, 0x50, 0x00, 0x00, 0x00, 0x00,
      ])
    ...
    it('should parse JPEG dimensions (128x128)', () => {
      const result = getImageDimensions(createMinimalJPEG(128, 128))
      expect(result).toEqual({ width: 128, height: 128, type: 'jpg' })
    })

    it('should handle malformed JPEG gracefully', () => {
      // JPEG with no valid SOF0 marker
    ...
    it('should skip non-0xFF bytes in JPEG parsing', () => {
      const jumpBytes = new Uint8Array([
        0xff,
        0xd8,
        0xff, // Valid SOI signature (first 3 bytes checked by code: 0xFF, 0xD8, 0xFF)
        0xfe, // COM marker, evaluated as uint8[3]
        0x00,
        0x04, // length = 4
        0x00,
        0x00, // payload of COM
        0x00,
        0x00, // Dummy non-0xFF bytes to trigger offset++ skip logic
        0xff,
        0xc0, // Valid SOF0 marker
        0x00,
        0x11,
        0x08,
        0x00,
      ...
      const result = getImageDimensions(jumpBytes)
      expect(result).toEqual({ width: 64, height: 64, type: 'jpg' })
    })
