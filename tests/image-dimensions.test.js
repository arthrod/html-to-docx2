...
function createMalformedWebP() {
  const data = new Uint8Array([
    ...
    0x38, 0x58,
  ]) // 'VP8X' instead of VP8 or VP8L
  return data
}

function createWebPWithoutStartCode() {
  const data = new Uint8Array(40)
  const dv = new DataView(data.buffer)

  dv.setUint32(0, 0x52494646, false)
  dv.setUint32(4, 32, true)
  dv.setUint32(8, 0x57454250, false)

  dv.setUint32(12, 0x56503820, false) // 'VP8 '
  dv.setUint32(16, 10, true)

  dv.setUint8(23, 0x00) // Not 0x9D

  return data
}

function createMinimalWebP_VP8L_Fallback() {
  const data = new Uint8Array(40)
  const dv = new DataView(data.buffer)
  ...
  dv.setUint8(20, 0x20) // signature != 0x2F
...
describe('Image Dimensions', () => {
  describe('getImageDimensions', () => {
    ...
    it('should handle malformed WebP fallback', () => {
      const result = getImageDimensions(createMalformedWebP())
      expect(result).toEqual({ width: 100, height: 100, type: 'webp' })
    })

    it('should handle WebP VP8 missing start code fallback', () => {
      const result = getImageDimensions(createWebPWithoutStartCode())
      expect(result).toEqual({ width: 0, height: 0, type: 'webp' })
    })

    it('should handle WebP VP8L invalid signature fallback', () => {
      const result = getImageDimensions(createMinimalWebP_VP8L_Fallback())
      expect(result).toEqual({ width: 1, height: 1, type: 'webp' })
    })
  })
})
