...
function createMinimalJPEG(width = 64, height = 64) {
  ...
  sof0Dv.setUint16(2, 15, false) // length
  ...
  sof0Dv.setUint8(15, 2) // component ID
  sof0Dv.setUint8(16, 0x11) // sampling
...
function createMinimalBMP(width = 48, height = 48) {
  const data = new Uint8Array(54) // 54 bytes total
  const dv = new DataView(data.buffer)

  // BMP header (14 bytes)
  dv.setUint16(0, 0x4d42, false) // 'BM' big-endian (data[0]=0x42, data[1]=0x4D)
  ...
  dv.setUint32(18, width, true) // width (32-bit)
  dv.setUint32(22, height, true) // height (32-bit)
  ...
  dv.setUint32(50, 0, true) // colors important
...
describe('Image Dimensions', () => {
  describe('getImageDimensions', () => {
    ...
    it('should skip non-marker bytes in JPEG', () => {
      // SOI, APP0 marker with length 2, then a non-marker byte, then SOF0
      const buffer = new Uint8Array([
        0xff, 0xd8, 0xff, 0xe0, 0x00, 0x02, 0x00, 0xff, 0xc0, 0x00, 0x11, 0x08, 0x00, 0x40,
        0x00, 0x40,
      ])
