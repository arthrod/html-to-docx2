...
function createMinimalJPEG(width = 64, height = 64) {
  ...
  dv.setUint16(4, 16, false) // length (includes the 2 bytes for length)
  ...
  // SOF0 marker
  const sof0 = new Uint8Array(19)
  const sof0Dv = new DataView(sof0.buffer)
  sof0Dv.setUint16(0, 0xffc0, false) // SOF0
  sof0Dv.setUint16(2, 17, false) // length (8 + components*3)
  ...
  sof0Dv.setUint8(15, 0) // quantization table
  sof0Dv.setUint8(16, 3) // component ID
  sof0Dv.setUint8(17, 0x11) // sampling
  sof0Dv.setUint8(18, 0) // quantization table
...
// Minimal valid WebP (30x30) - VP8 format (Lossy)
function createMinimalLossyWebP(width = 30, height = 30) {
  ...
  // VP8 image data (frame header starts at offset 23)
  // According to specification:
  // Offset 23, 24, 25: 0x9D 0x01 0x2A (Start code)
  dv.setUint8(23, 0x9d)
  dv.setUint8(24, 0x01)
  dv.setUint8(25, 0x2a)

  // Offset 26, 27: Width (14 bits) + horizontal scale (2 bits)
  dv.setUint16(26, width & 0x3fff, true)

  // Offset 28, 29: Height (14 bits) + vertical scale (2 bits)
...
describe('Image Dimensions', () => {
  describe('getImageDimensions', () => {
    ...
    it('should fallback to default for malformed JPEG without SOF', () => {
      const malformedJpeg = new Uint8Array([
        0xff, 0xd8, 0xff, 0x00, 0x01, 0x02, 0x03, 0xff, 0xd9,
      ])
      const result = getImageDimensions(malformedJpeg)
      expect(result).toEqual({ width: 100, height: 100, type: 'jpg' })
    })

    it('should handle JPEG with non-marker bytes between segments', () => {
      // Create a valid JPEG but insert non-marker bytes (padding) before a marker
      const validJpeg = createMinimalJPEG(64, 64)
      const paddedJpeg = new Uint8Array(validJpeg.length + 3)
      paddedJpeg.set(validJpeg.subarray(0, 20)) // copy up to APP0 end
      paddedJpeg.set([0x00, 0x00, 0x00], 20) // insert padding
      paddedJpeg.set(validJpeg.subarray(20), 23) // copy the rest

      const result = getImageDimensions(paddedJpeg)
      expect(result).toEqual({ width: 64, height: 64, type: 'jpg' })
    })
    ...
    it('should parse WebP (VP8L Lossless) dimensions (24x24)', () => {
      const result = getImageDimensions(createMinimalWebP(24, 24))
      expect(result).toEqual({ width: 24, height: 24, type: 'webp' })
    })

    it('should parse WebP (VP8 Lossy) dimensions (30x30)', () => {
      const result = getImageDimensions(createMinimalLossyWebP(30, 30))
      expect(result).toEqual({ width: 30, height: 30, type: 'webp' })
    })

    it('should fallback to default for malformed WebP without VP8/VP8L chunk', () => {
