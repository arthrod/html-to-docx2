...
function createMinimalJPEG(width = 64, height = 64, marker = 0xc0) {
  ...
  sof0Dv.setUint16(0, 0xff00 | marker, false) // SOF0/1/2
...
// Minimal valid lossy WebP (24x24) - VP8 format
function createLossyWebP(width = 24, height = 24) {
  const data = new Uint8Array(40)
  const dv = new DataView(data.buffer)
  ...
  // VP8 chunk
  dv.setUint32(12, 0x56503820, false) // 'VP8 ' (with trailing space)
  dv.setUint32(16, 14, true) // chunk size

  // VP8 frame header starts with 3 bytes (uncompressed frame tag)
  dv.setUint8(20, 0x00) // key frame, version 0, show frame
  dv.setUint8(21, 0x00) // First partition size
  dv.setUint8(22, 0x00) // First partition size

  // VP8 start code (0x9d012a)
  dv.setUint8(23, 0x9d)
  dv.setUint8(24, 0x01)
  dv.setUint8(25, 0x2a)
...
describe('Image Dimensions', () => {
  describe('getImageDimensions', () => {
    ...
    it('should parse JPEG dimensions with SOF0', () => {
      const result = getImageDimensions(createMinimalJPEG(64, 64, 0xc0))
      expect(result).toEqual({ width: 64, height: 64, type: 'jpg' })
    })

    it('should parse JPEG dimensions with SOF1', () => {
      const result = getImageDimensions(createMinimalJPEG(128, 256, 0xc1))
      expect(result).toEqual({ width: 128, height: 256, type: 'jpg' })
    })

    it('should parse JPEG dimensions with SOF2', () => {
      const result = getImageDimensions(createMinimalJPEG(800, 600, 0xc2))
      expect(result).toEqual({ width: 800, height: 600, type: 'jpg' })
    })

    it('should handle malformed JPEG gracefully', () => {
      // Just SOI marker, no SOF
      const malformed = new Uint8Array([0xff, 0xd8, 0xff, 0x00])
    ...
    it('should parse lossy WebP dimensions', () => {
      const result = getImageDimensions(createLossyWebP(800, 600))
      expect(result).toEqual({ width: 800, height: 600, type: 'webp' })
    })

    it('should handle malformed WebP gracefully', () => {
      // Valid RIFF/WEBP header but missing/invalid chunks
      const malformed = new Uint8Array([
        ...
        0x50,
        0x38,
        0x58, // VP8X (unsupported format for this parser currently)
      ])
      const result = getImageDimensions(malformed)
