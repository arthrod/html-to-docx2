...
describe("ListStyleBuilder", () => {
  ...
  const customBuilder = new ListStyleBuilder({
    defaultOrderedListStyleType: "lowerLetter",
  });
  ...
  describe("getListStyleType", () => {
    test("should return correct DOCX list styles", () => {
      ...
      expect(builder.getListStyleType("upper-alpha-bracket-end")).toBe(
        "upperLetter",
      );
      expect(builder.getListStyleType("lower-alpha")).toBe("lowerLetter");
      expect(builder.getListStyleType("lower-alpha-bracket-end")).toBe(
        "lowerLetter",
      );
  ...
  describe("getListPrefixSuffix", () => {
    test("should return dot suffix for roman/alpha styles", () => {
      expect(
        builder.getListPrefixSuffix({ "list-style-type": "upper-roman" }, 0),
      ).toBe("%1.");
      expect(
        builder.getListPrefixSuffix({ "list-style-type": "lower-alpha" }, 1),
      ).toBe("%2.");
    });

    test("should return bracket-end suffix", () => {
      expect(
        builder.getListPrefixSuffix(
          { "list-style-type": "decimal-bracket-end" },
          0,
        ),
      ).toBe("%1)");
      expect(
        builder.getListPrefixSuffix(
          { "list-style-type": "upper-alpha-bracket-end" },
          0,
        ),
      ).toBe("%1)");
      expect(
        builder.getListPrefixSuffix(
          { "list-style-type": "lower-alpha-bracket-end" },
          0,
        ),
      ).toBe("%1)");
    });
    ...
    test("should return bracket wrapper", () => {
      expect(
        builder.getListPrefixSuffix(
          { "list-style-type": "decimal-bracket" },
          0,
        ),
      ).toBe("(%1)");
    });
  ...
  describe("getUnorderedListPrefixSuffix", () => {
    test("should return correct bullet types", () => {
      expect(
        ListStyleBuilder.getUnorderedListPrefixSuffix({
    ...
    test("should handle null/undefined", () => {
      ...
      const undefResult =
        ListStyleBuilder.getUnorderedListPrefixSuffix(undefined);
...
describe("Image to base64 utilities", () => {
  ...
  describe("imageToBase64", () => {
    test("should reject non-http protocols", async () => {
      await expect(
        imageToBase64("ftp://example.com/image.png"),
