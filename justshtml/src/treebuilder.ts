...
export class TreeBuilder {
  ...
  constructor(fragment_context: FragmentContext | null = null, iframe_srcdoc = false, collect_errors = false) {
  ...
  _has_element_in_scope(target: string | Set<string>, terminators: Set<string> | null = null, checkIntegrationPoints = true) {
    ...
    for (let idx = this.open_elements.length - 1; idx >= 0; idx -= 1) {
      ...
      const matchesTarget = typeof target === "string" ? node.name === target : target.has(node.name);
