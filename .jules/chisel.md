...
## 2025-05-18 - Legacy Ad-Hoc DOM Types
**Learning:** Legacy projects often re-declare third-party interfaces as loose local aliases (e.g., `type VNodeType = { [key: string]: any }`) because the original migration lacked type guards or strict compiler flags.
**Action:** Always delete loose local aliases for known classes and enforce the usage of strict imported class definitions alongside native type guards (like `isVNode` or the `in` operator) to natively narrow unions without brute-forcing casts.
