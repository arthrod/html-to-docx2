...
## 2026-06-09 - Remove ad-hoc type aliases that shadow real types
**Learning:** Sometimes codebase migration includes ad-hoc type definitions (e.g. `type VNodeType = { tagName: string }`) that shadow the true classes or types (`export class VNode`). This forces repetitive casts (`as VNodeType`) and prevents native type guards (`isVNode(vNode)`) from properly inferring properties on the object, creating a false layer of typing that hides the actual implementation.
**Action:** When you spot ad-hoc interface-like definitions that closely match an imported class (and native type guards are available), delete the ad-hoc types, import the real classes, and remove all redundant casts. Make sure arrays mapping to `(ClassA | ClassB)[]` do not cause `Property does not exist` errors when accessed directly without further checks.
