...
## 2025-02-14 - [HTML Input Typing]
**Learning:** In the `src/vendor/justjshtml` directory, core HTML input parameters for parser and tokenizer functions (like `parseDocument`, `Tokenizer.run`, and `Tokenizer.initialize`) were incorrectly typed as `any`. Upstream logic (e.g., in `justhtml.ts`) handles decoding `ArrayBuffer` and `Uint8Array` inputs into strings before passing them down to these lower-level functions, making it safe to narrow the type down to `string`.
**Action:** When typing legacy JS parsers, track the data flow from the public API boundary to internal methods. Often, the public API handles complex type coercion (e.g., buffer to string), allowing internal methods to be strictly typed with the coerced result (e.g., `string`).
