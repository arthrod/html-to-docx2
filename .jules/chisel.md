...
## 2024-05-24 - TS(7023) Recursive Function Return Type
**Learning:** `TS(7023)` ("implicitly has return type 'any' because it does not have a return type annotation and is referenced directly or indirectly in one of its return expressions") is very common in complex mutually recursive logic, like DOM parsing or CSS selector matching.
**Action:** Always provide explicit return types (like `: boolean` or `: string`) on the function signatures of recursive methods to satisfy the type checker and remove `@ts-expect-error` directives safely without needing to touch internal implementation logic.
