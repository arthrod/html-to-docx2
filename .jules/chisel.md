...
## 2026-06-29 - Removing Redundant Ad-Hoc Types Eclipsing Valid Core Types
**Learning:** Legacy JS codebases often re-declare dummy index-signature types (like `type VNodeType = { [key: string]: string | number | boolean | ... }`) in helper files, casting actual typed objects to these dummies to bypass strict property checks during early migration. This breaks control flow analysis, as the compiler is forced to abandon the true `VNode` shape for the loose index signature, requiring rampant `as VNodeType` assertions everywhere.
**Action:** When finding types declared loosely as `{ [key: string]: ... }` masking core classes, immediately investigate if a native type exists (e.g., `VNode` from `vdom/index`). Delete the ad-hoc type, import the true class, and delete the redundant `as` assertions. TypeScript's native type guards (e.g., `isVNode()`) will correctly narrow the type natively without needing unsafe casts.
