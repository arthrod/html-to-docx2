...
## 2024-05-30 - Implicit Any Returns in Mutual/Direct Recursion
**Learning:** When recursive methods like `matches`, `_matchesSimple`, and `_matchesPseudo` interact with each other without explicit return type annotations, TypeScript's compiler (throwing TS7023) is unable to infer their types due to cyclic dependencies.
**Action:** Always provide explicit return types (like `: boolean`) for recursive functions to satisfy the compiler and remove `@ts-expect-error` bandages without affecting runtime behavior.
