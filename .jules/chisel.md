...
## 2026-05-31 - Recursive Type Inference Blocks
**Learning:** TypeScript cannot implicitly infer the return types of interdependent or mutually recursive functions (e.g., `matches`, `_matchesSimple`, `_matchesPseudo` in `SelectorMatcher`). This results in `TS(7023)` and implicit `any` return types, which migration scripts often mask with `@ts-expect-error`.
**Action:** When encountering `TS(7023)` on interdependent matcher/parser functions, explicitly annotate the return type (e.g., `: boolean`) to break the inference cycle. This satisfies the compiler and restores static verification without altering runtime behavior.
