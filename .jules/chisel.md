...
## 2025-02-18 - Missing Type Imports Cause TS Regressions
**Learning:** When resolving `any` array types (e.g. `errors: any[]`) with proper concrete types (e.g. `ParseError[]`) across multiple files, forgetting to import the newly referenced class/type (`import { ParseError } from './tokens.js'`) in every modified file will cause TypeScript compilation failures (`TS2304: Cannot find name`).
**Action:** Always verify that every file modified to use a new type explicitly imports that type, particularly when using ad-hoc script replacements that don't benefit from IDE auto-imports. Test this systematically using `tsc --noEmit`.
