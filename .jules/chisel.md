...
## 2024-05-24 - Typing Union Errors in Legacy Classes
**Learning:** When typing an `error` parameter or property that intercepts exceptions and accesses non-standard properties (like `error?.code`), and you want to union it with the standard `Error` interface to be safe, you cannot simply do `Error | { code?: string }`. Because `code` doesn't exist on the standard `Error` type, TypeScript will throw an error `TS2339` when trying to evaluate `error?.code`.
**Action:** You must intersect `Error` with the custom properties (e.g., `(Error & { code?: string })`) so the compiler allows accessing those properties across all members of the union that might supply them.
