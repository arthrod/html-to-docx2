...
## 2024-06-25 - ES Modules and mid-file imports
**Learning:** In ES Modules, while hoisting allows mid-file imports to work syntactically, many linting setups (including `eslint-plugin-import`) enforce that all `import` statements appear at the very top of the file before any other statements. Adding imports dynamically in the middle of files during TS migration refactoring will trigger code review failures or lint errors.
**Action:** When adding necessary type imports (e.g., pulling in `ParseError` or `Node`) using ad-hoc `fs.readFileSync` scripts, always append them to the beginning of the file content string instead of substituting them inline over existing mid-file statements.
