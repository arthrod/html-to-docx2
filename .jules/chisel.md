...
## 2025-05-24 - Typed error and root Node in JustHTML
**Learning:** Legacy JavaScript parser roots implicitly assume full DOM node support and typically fall back to `any` during migration. Also, `error` in `StrictModeError` and `JustHTML.errors` was typed as `any`, masking the actual `ParseError` which implements the `Error` interface.
**Action:** When migrating HTML parsers, explicitly type root elements using the custom `Node` implementation (e.g., from `./node.js`) rather than `any`. Ensure custom error definitions like `ParseError` intersect with the native `Error` type if standard error properties (like `code`) are accessed dynamically.
