...
## 2026-07-03 - Avoid array spread operator in HTML parsing
**Learning:** In V8/Bun hot paths, using `Array.push(...items)` to merge child arrays introduces call stack size risks for large nodes and is significantly slower than using a standard `for` loop to push items individually.
**Action:** Avoid `Array.push(...items)` in hot loops for processing HTML ASTs (like `normalizeDocumentRootNodes`); use a standard `for` loop instead.
