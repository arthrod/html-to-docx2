...
## 2025-02-12 - Prevent Maximum call stack size exceeded when merging VNodes
**Learning:** In `src/helpers/html-parser.ts`, using the array spread operator (`normalizedNodes.push(...bodyNode.children)`) on deeply nested HTML DOM structures can exceed V8 engine call stack limits if a node contains tens of thousands of children.
**Action:** Replace `Array.push(...items)` with a `for` loop to serialize elements individually when flattening Virtual DOM arrays to improve performance and prevent crashes on large HTML payloads.
