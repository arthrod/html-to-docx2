...
## 2024-05-24 - Array Spread Operator Bottleneck
**Learning:** Using the spread operator (`Array.push(...items)`) in V8/Bun hot paths (like DOM parsing) can cause 'Maximum call stack size exceeded' errors with large trees.
**Action:** Always use standard `for` loops instead of spread operators when pushing items to arrays in processing-heavy logic.
