...
## 2024-06-28 - Use `escapeXml` for XML/SVG escaping instead of chained `.replace()` calls

**Learning:** The `escapeXml` function implements a highly optimized single-pass string concatenation for XML escaping that is significantly faster than using sequential chained `.replace()` calls for XML/SVG serialization. In `serializeVNodeToSVG` in `src/helpers/render-document-file.ts`, multiple chained `.replace` calls were used for escaping text and attributes, which creates unnecessary intermediate strings.
**Action:** Replace chained `.replace()` calls with a single call to `escapeXml` for XML/SVG serialization to improve performance. Remember to explicitly cast values using `String(value)` when passing to `escapeXml` to ensure type safety.
