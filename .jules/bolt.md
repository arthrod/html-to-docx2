## 2026-07-16 - Single-Pass String Iteration Outperforms RegExp chained replace in Bun/V8
...
## 2026-07-16 - Single RegExp Outperforms Multiple replace calls with Array.forEach
...
## 2026-07-16 - Avoid array spread operator in hot paths
**Learning:** In V8/Bun hot paths, merging fragment arrays using `Array.push(...items)` introduces call stack size risks for large documents and is significantly slower than using a standard `for` loop to push items individually.
**Action:** Avoid `Array.push(...items)` in tight parsing or rendering loops (e.g., merging nodes in `src/helpers/html-parser.ts`); use a standard `for` loop instead.
