...
def apply_diff_to_paragraph(doc, para_elem, baseline_runs, current_runs, operations):
        ...
        if op == 'equal':
            ...
            while remaining > 0 and current_run_idx < len(current_runs):
                ...
                if available <= remaining:
                    ...
                    insert_xml = f'<w:ins><w:r>{rpr}<w:t xml:space="preserve">{escape_xml(chunk_to_insert)}</w:t></w:r></w:ins>'
...
def main():
    if len(sys.argv) < 4:
        print("Usage: python scripts/diff-redline.py <baseline.docx> <current.docx> <output.docx>")
        sys.exit(1)
