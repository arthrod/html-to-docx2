...
def unpack_docx(docx_path, output_dir):
    ...
    if "Suggested RSID" in result.stdout:
        rsid = result.stdout.split("Suggested RSID for edit session: ")[1].strip()
        return rsid
    return None
...
def parse_runs_from_paragraph(para_elem):
    ...
    for run in para_elem.getElementsByTagName("w:r"):
        ...
        for dt_elem in run.getElementsByTagName("w:delText"):
            if dt_elem.firstChild:
                text_parts.append(dt_elem.firstChild.data)
...
def compare_paragraphs(doc, baseline_paras, current_paras):
    ...
    for tag, i1, i2, j1, j2 in matcher.get_opcodes():
        if tag == 'equal':
            ...
            for j in range(j1, j2):
                ...
                runs_xml = []
                for run in current_runs:
                    runs_xml.append(f'<w:r>{run["rPr"]}<w:t xml:space="preserve">{escape_xml(run["text"])}</w:t></w:r>')
...
def main():
    if len(sys.argv) < 4:
        print("Usage: python scripts/diff-redline-v2.py <baseline.docx> <current.docx> <output.docx>")
        sys.exit(1)
    ...
    print("📊 DOCX Redlining with Style Preservation")
    print("    Using official pack.py and Document library\n")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        rsid_current = unpack_docx(current_docx, current_unpacked)
        print("✓ Extraction complete")
        if rsid_baseline:
            print(f"  Suggested RSID: {rsid_baseline}\n")
