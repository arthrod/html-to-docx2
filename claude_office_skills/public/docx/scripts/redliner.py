...
def _merge_styles(old_doc: Document, new_doc: Document) -> None:
    ...
    def _find_child(root, tag_name):
        for child in root.childNodes:
            if child.nodeType == child.ELEMENT_NODE and child.tagName == tag_name:
                return child
        return None

    for singleton in ('w:docDefaults', 'w:latentStyles'):
        existing = _find_child(new_root, singleton)
        if existing is None:
            source = _find_child(old_root, singleton)
            if source is not None:
                imported_singleton = new_root.ownerDocument.importNode(source, deep=True)
                if new_root.firstChild is not None:
                    new_root.insertBefore(imported_singleton, new_root.firstChild)
                else:
                    new_root.appendChild(imported_singleton)
