    ...
    def suggest_deletion(self, elem):
        ...
        if elem.nodeName == 'w:r':
            ...
            if elem.getElementsByTagName('w:delText'):
                msg = 'w:r element already contains w:delText'
                raise ValueError(msg)
        ...
        if elem.nodeName == 'w:p':
            ...
            if elem.getElementsByTagName('w:ins') or elem.getElementsByTagName('w:del'):
                msg = 'w:p element already contains tracked changes'
                raise ValueError(msg)
        ...
        msg = f'Element must be w:r or w:p, got {elem.nodeName}'
        raise ValueError(msg)
...
class Document:
    ...
    def reply_to_comment(self, parent_comment_id: int, text: str) -> int:
        ...
        if parent_comment_id not in self.existing_comments:
            msg = f'Parent comment with id={parent_comment_id} not found'
            raise ValueError(msg)
    ...
    def validate(self) -> None:
        ...
        if not redlining_validator.validate():
            msg = 'Redlining validation failed'
            raise ValueError(msg)
  ...
  <w:commentReference w:id="{comment_id}"/>
    ...
    def _add_author_to_people(self, author) -> None:
        ...
        if not people_path.exists():
            msg = 'people.xml should exist after _setup_tracking'
            raise ValueError(msg)
