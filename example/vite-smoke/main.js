...
const setStatus = (value, details = '') => {
  ...
  statusEl.className = value.startsWith('PASS') ? 'pass' : value.startsWith('FAIL') ? 'fail' : '';
...
const runSmoke = async () => {
  try {
    ...
    setStatus('PASS_BLOB', `Generated ${size} bytes (${Object.prototype.toString.call(result)})`);
    console.log('[SMOKE] PASS_BLOB', { size, rawType: Object.prototype.toString.call(result) });
  } catch (error) {
