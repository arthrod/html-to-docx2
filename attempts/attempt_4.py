...
def _iter_runs_with_wrappers(p: etree._Element):
    """Yield (run, hyperlink_wrapper_or_None) for all runs in a paragraph.

    We flatten hyperlinks and existing tracked changes so that formatting and
    relationships (e.g., the hyperlink relId) can be reapplied when we rebuild
    the paragraph. The wrapper element is deep-copied by callers before use to
    avoid mutating the original tree.
    """

    def _yield_from(parent: etree._Element, wrapper: etree._Element | None = None):
        for child in parent:
            if child.tag == qn('w:r'):
                yield child, wrapper
            elif child.tag == qn('w:hyperlink'):
                # Preserve hyperlink context for contained runs
                yield from _yield_from(child, child)
            elif child.tag in {qn('w:ins'), qn('w:del')}:
                # Flatten existing tracked changes but keep wrapper context
                yield from _yield_from(child, wrapper)
            else:
                # Unknown children are ignored here; they'll surface as 'other'
                # tokens when we encounter their runs.
                continue

    yield from _yield_from(p, None)


def _wrap_if_needed(node: etree._Element, wrapper: etree._Element | None) -> etree._Element:
    """Return node wrapped in a copy of `wrapper` if provided."""
    if wrapper is None:
        return node
    hl = deepcopy(wrapper)
...
def paragraph_runs_tokens(p: etree._Element):
    ...
    A token is dict with:
        ...
        wrapper: optional hyperlink element that should wrap reconstructed runs
    """
    tokens = []
    for r, wrapper in _iter_runs_with_wrappers(p):
        rPr = r.find(qn('w:rPr'))
        ...
        if t_nodes:
            ...
            tokens.append({'kind': 'text', 'text': full_text, 'rPr': rPr_copy, 'run_xml': r, 'wrapper': wrapper})
            continue

        # Tabs/br handled explicitly
        if r.find(qn('w:tab')) is not None:
            tokens.append({'kind': 'tab', 'text': '\t', 'rPr': rPr_copy, 'run_xml': r, 'wrapper': wrapper})
            continue
        if r.find(qn('w:br')) is not None:
            tokens.append({'kind': 'br', 'text': '\n', 'rPr': rPr_copy, 'run_xml': r, 'wrapper': wrapper})
            continue

        # Catch-all for other run content (drawing, fldChar, etc.)
        tokens.append({'kind': 'other', 'text': '', 'rPr': rPr_copy, 'run_xml': r, 'wrapper': wrapper})
...
def diff_run_text_charlevel(
    ...
    cidgen: ChangeIdGen,
    *,
    new_wrapper: etree._Element | None,
    old_wrapper: etree._Element | None,
):
    ...
    for tag, i1, i2, j1, j2 in sm.get_opcodes():
        if tag == 'equal':
            if i2 > i1:
                out.append(_wrap_if_needed(clone_r_with_text(old_text[i1:i2], new_rPr, deleted=False), new_wrapper))
        elif tag == 'delete':
            if i2 > i1:
                cid = cidgen.next()
                de = make_del_container(author, date_iso, cid)
                de.append(_wrap_if_needed(clone_r_with_text(old_text[i1:i2], old_rPr, deleted=True), old_wrapper))
                out.append(de)
        elif tag == 'insert':
            if j2 > j1:
                cid = cidgen.next()
                ins = make_ins_container(author, date_iso, cid)
                ins.append(_wrap_if_needed(clone_r_with_text(new_text[j1:j2], new_rPr, deleted=False), new_wrapper))
                out.append(ins)
        elif tag == 'replace':
            if i2 > i1:
                cid = cidgen.next()
        ...
        if tag == 'equal':
            ...
            for k in range(len(old_slice)):
                ...
                if o['kind'] in {'tab', 'br'}:
                    ...
                    out_p.append(_wrap_if_needed(r, n['wrapper']))
                elif o['kind'] == 'text':
                    ...
                    out_p.append(_wrap_if_needed(deepcopy(n['run_xml']), n['wrapper']))
            ...
            for o in old_slice:
                if o['kind'] == 'text':
                    de.append(_wrap_if_needed(clone_r_with_text(o['text'], o['rPr'], deleted=True), o['wrapper']))
                elif o['kind'] in {'tab', 'br'}:
                    de.append(_wrap_if_needed(clone_r_special(o['kind'], o['rPr'], deleted=True), o['wrapper']))
                else:
                    ...
                    de.append(_wrap_if_needed(deepcopy(o['run_xml']), o['wrapper']))
            out_p.append(de)
            ...
            for n in new_slice:
                if n['kind'] == 'text':
                    ins.append(_wrap_if_needed(clone_r_with_text(n['text'], n['rPr'], deleted=False), n['wrapper']))
                elif n['kind'] in {'tab', 'br'}:
                    ins.append(_wrap_if_needed(clone_r_special(n['kind'], n['rPr'], deleted=False), n['wrapper']))
                else:
                    ins.append(_wrap_if_needed(deepcopy(n['run_xml']), n['wrapper']))
            out_p.append(ins)
            ...
            if (
                ...
                pieces = diff_run_text_charlevel(
                    ...
                    date_iso,
                    cidgen,
                    new_wrapper=new_slice[0]['wrapper'],
                    old_wrapper=old_slice[0]['wrapper'],
                )
                for node in pieces:
                    out_p.append(node)
                ...
                if old_slice[0]['text'] == new_slice[0]['text'] and not equal_runs_style(
                    ...
                    out_p.append(_wrap_if_needed(r, new_slice[0]['wrapper']))
            else:
