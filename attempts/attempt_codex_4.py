...
index 0000000000000000000000000000000000000000..b45c5b40415ee70c1e345d6642b730587f677d3d
--- /dev/null
...
+    """Ensure settings.xml enables <w:trackRevisions/> with a compliant XML prolog."""
+
+    if settings_bytes:
+        try:
...
+    """Return True when both paragraphs have identical token texts and run properties."""
+
...
+    attrs = {W_ATTR('id'): str(next_id)}
+    if author:
+        attrs[W_ATTR('author')] = author
+    if date:
+        attrs[W_ATTR('date')] = date
+        if 'w16du' in NS:
+            attrs[f"{{{NS['w16du']}}}dateUtc"] = date
+    ch = ET.Element(W('rPrChange'), attrs)
+    payload = ET.SubElement(ch, W('rPr'))
+    if old_rPr is not None:
+        for child in list(old_rPr):
+            cloned_child = _clone(child)
+            if cloned_child is not None:
+                payload.append(cloned_child)
+    run_el.append(ch)
+
+
