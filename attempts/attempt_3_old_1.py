...
def text_of_element(elem: etree._Element) -> str:
    ...
    t_elements: list[etree._Element] = elem.xpath('.//w:t', namespaces=NSMAP)  # type: ignore[assignment]
    texts = [t.text or '' for t in t_elements]
    return ''.join(texts)


def block_iter(body: etree._Element) -> Generator[tuple[etree._Element, str], None, None]:
...
def paragraph_runs_tokens(p: etree._Element) -> list[dict]:
    ...
    """
    tokens = []
    run_elements: list[etree._Element] = p.xpath('./w:r', namespaces=NSMAP)  # type: ignore[assignment]
