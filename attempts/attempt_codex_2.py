...
diff --git a/attempts/attempt_10_track_styles.py b/attempts/attempt_10_track_styles.py
new file mode 100644
index 0000000000000000000000000000000000000000..8c72fefe9dbd1ab89cf2512c72d972b2e63db71e
--- /dev/null
+++ b/attempts/attempt_10_track_styles.py
@@ -0,0 +1,637 @@
+#!/usr/bin/env python3
...
+    """Register namespace prefixes from XML content for stable serialization."""
+    nsmap: dict[str, str] = {}
+    for event, (prefix, uri) in ET.iterparse(io.BytesIO(xml_bytes), events=('start-ns',)):
+        if prefix is None:
+            prefix = ''
+        nsmap[prefix] = uri
...
+
+
+def _read_docx_all(docx_path: str) -> tuple[ET.Element, Optional[ET.Element], dict[str, bytes], dict[str, str]]:
...
+    doc_nsmap = _register_all_namespaces(doc_bytes)
+    doc = ET.fromstring(doc_bytes)
+
+    styles: Optional[ET.Element] = None
+    if 'word/styles.xml' in files:
+        styles_bytes = files['word/styles.xml']
+        _register_all_namespaces(styles_bytes)
+        styles = ET.fromstring(styles_bytes)
+    return doc, styles, files, doc_nsmap
+
+
+def _ensure_track_revisions(settings_bytes: Optional[bytes]) -> bytes:
+    """Ensure settings.xml has <w:trackRevisions/> enabled with compliant prolog."""
+    if settings_bytes:
+        try:
...
+
+
+def _sanitize_rPr_clone(rPr: Optional[ET.Element]) -> Optional[ET.Element]:
+    cloned = _clone(rPr)
+    if cloned is None:
+        return None
...
+
+
+def _sanitize_pPr_clone(pPr: Optional[ET.Element]) -> Optional[ET.Element]:
+    cloned = _clone(pPr)
+    if cloned is None:
+        return None
+    rPr = cloned.find('w:rPr', NS)
+    if rPr is not None:
...
+        self.text = text
+        self.rPr = _sanitize_rPr_clone(rPr)
+
+
+class Para:
...
+        self.el = el
+        self.pPr = _sanitize_pPr_clone(pPr)
+        self.tokens = tokens
...
+    if rPr is not None:
+        cloned_rPr = _sanitize_rPr_clone(rPr)
...
+    if pPr is not None:
+        cloned_pPr = _sanitize_pPr_clone(pPr)
+        if cloned_pPr is not None:
+            target_p.insert(0, cloned_pPr)
+
+
+def _copy_paragraph_attrib(target_p: ET.Element, source_p: Optional[ET.Element]) -> None:
+    """Copy paragraph-level attributes (like w14:paraId) from source to target."""
+    if source_p is None:
+        return
+    for key, value in source_p.attrib.items():
+        target_p.set(key, value)
+
+
...
+    newp = ET.Element(W('p'))
+    _copy_paragraph_attrib(newp, p_old)
...
+
+
+def _same_xml(a: Optional[ET.Element], b: Optional[ET.Element]) -> bool:
...
+    return _same_xml(a, b)
+
+
+def _append_rPr_change(container: ET.Element, old_rPr: Optional[ET.Element], author: str, date: str, change_id: int) -> None:
+    change = ET.Element(W('rPrChange'))
+    change.set(W_ATTR('id'), str(change_id))
+    if author:
+        change.set(W_ATTR('author'), author)
+    if date:
+        change.set(W_ATTR('date'), date)
+        if 'w16du' in NS:
+            change.set(f"{{{NS['w16du']}}}dateUtc", date)
+
+    payload = _clone(old_rPr) if old_rPr is not None else ET.Element(W('rPr'))
+    if payload is None:
+        payload = ET.Element(W('rPr'))
+    change.append(payload)
+
...
+    """Append <w:rPrChange> capturing the prior run properties."""
+    rPr = run_el.find('w:rPr', NS)
+    if rPr is None:
+        rPr = ET.SubElement(run_el, W('rPr'))
+
+    _append_rPr_change(run_el, old_rPr, author, date, next_id)
+
+
+def _maybe_add_paragraph_rpr_change(
+    para_el: ET.Element,
+    old_para: Para,
+    new_para: Para,
+    author: str,
...
+    next_id: int,
+) -> int:
+    new_pPr = para_el.find('w:pPr', NS)
+    old_pPr = old_para.pPr
+    if new_pPr is None:
+        return next_id
+
+    new_rPr = new_pPr.find('w:rPr', NS)
+    old_rPr = old_pPr.find('w:rPr', NS) if old_pPr is not None else None
+    if _same_xml(old_rPr, new_rPr):
+        return next_id
+
+    if new_rPr is None:
+        new_rPr = ET.SubElement(new_pPr, W('rPr'))
+
+    _append_rPr_change(new_rPr, old_rPr, author, date, next_id)
+    return next_id + 1
+
+
+def _para_tokens_equivalent(old_para: Para, new_para: Para) -> bool:
+    """Return True when both paragraphs have identical tokens and formatting."""
+    if len(old_para.tokens) != len(new_para.tokens):
+        return False
+    for tok_old, tok_new in zip(old_para.tokens, new_para.tokens):
+        if tok_old.text != tok_new.text:
+            return False
+        if not _same_rpr(tok_old.rPr, tok_new.rPr):
+            return False
+    if not _same_xml(old_para.pPr, new_para.pPr):
+        return False
+    return True
+
+
...
+    next_id = id_start
+    newp = ET.Element(W('p'))
+    _copy_paragraph_attrib(newp, p_new.el)
+    _append_pPr(newp, p_new.pPr if p_new.pPr is not None else p_old.pPr)
+    next_id = _maybe_add_paragraph_rpr_change(newp, p_old, p_new, author, date, next_id)
+
...
+                old_idx = i1 + offset
+                if _para_tokens_equivalent(old_paras[old_idx], new_paras[j]):
+                    plan_for_new[j] = ('as_is', None)
+                else:
+                    plan_for_new[j] = ('replace_1to1', old_idx)
+        elif tag == 'insert':
...
+
+    try:
+        old_doc, old_styles, _old_files, _old_nsmap = _read_docx_all(args.from_path)
+        new_doc, new_styles, new_files, new_nsmap = _read_docx_all(args.to_path)
