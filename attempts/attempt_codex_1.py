...
diff --git a/attempts/attempt_9_pro_new.py b/attempts/attempt_9_pro_new.py
new file mode 100755
index 0000000000000000000000000000000000000000..59893e48562a26124c7e8715dd870b8d95474792
--- /dev/null
+++ b/attempts/attempt_9_pro_new.py
@@ -0,0 +1,971 @@
...
+
+
+_NS_DECL_RE = re.compile(rb'xmlns:([A-Za-z0-9_]+)="([^"]+)"')
+
+
+def _register_namespaces_from_xml(xml_bytes: bytes) -> dict[str, str]:
+    """Register namespaces from raw XML so ElementTree preserves prefixes."""
+    mapping: dict[str, str] = {}
+    for prefix_b, uri_b in _NS_DECL_RE.findall(xml_bytes):
+        prefix = prefix_b.decode('utf-8')
...
+
+
+def _inject_namespace_decls(body: bytes, extra_ns: Optional[dict[str, str]]) -> bytes:
+    if not extra_ns:
+        return body
+    existing = {prefix.decode('utf-8') for prefix, _ in _NS_DECL_RE.findall(body)}
+    additions = []
+    for prefix, uri in extra_ns.items():
+        if prefix == 'xml' or prefix in existing:
+            continue
...
+    if idx == -1:
+        return body
+    return body[:idx] + b''.join(additions) + body[idx:]
+
+
+def serialize_xml(elem: ET.Element, extra_ns: Optional[dict[str, str]] = None) -> bytes:
...
+    """
+    body = ET.tostring(elem, encoding='utf-8', xml_declaration=False)
+    body = _inject_namespace_decls(body, extra_ns)
+    header = b'<?xml version="1.0" encoding="UTF-8" standalone="yes"?>\n'
+    return header + body
+
+
+def _ensure_track_revisions(settings_xml: Optional[bytes]) -> bytes:
+    """Guarantee <w:trackRevisions/> exists with compliant XML prolog."""
+    if settings_xml:
+        try:
+            root = ET.fromstring(settings_xml)
+        except ET.ParseError:
+            logger.warning("Malformed settings.xml, recreating")
+            root = ET.Element(qn('w:settings'))
+    else:
+        root = ET.Element(qn('w:settings'))
+
+    if root.tag != qn('w:settings'):
+        logger.warning("Unexpected settings.xml root, normalizing")
+        root = ET.Element(qn('w:settings'))
+
...
+
+
+def _para_tokens_equivalent(old_p: ET.Element, new_p: ET.Element) -> bool:
+    """Return True when paragraphs have identical token texts and formatting."""
+    old_tokens = paragraph_runs_tokens(old_p)
+    new_tokens = paragraph_runs_tokens(new_p)
+    if len(old_tokens) != len(new_tokens):
+        return False
+    for o_tok, n_tok in zip(old_tokens, new_tokens):
+        if o_tok['kind'] != n_tok['kind']:
+            return False
+        if o_tok['text'] != n_tok['text']:
+            return False
+        if not equal_runs_style(o_tok['rPr'], n_tok['rPr']):
+            return False
+    return True
+
+
...
+        if 'w16du' in NS:
+            rPrChange.set(f"{{{NS['w16du']}}}dateUtc", date_iso)
+
...
+    """
+    out_p = ET.Element(qn('w:p'), attrib=dict(new_p.attrib))
+
...
+
+
+def _ensure_pPr_rPr(para: ET.Element) -> ET.Element:
...
+    marker = ET.Element(qn('w:ins'))
+    marker.set(qn('w:id'), str(cidgen.next()))
+    if author:
+        marker.set(qn('w:author'), author)
+    if date_iso:
+        marker.set(qn('w:date'), date_iso)
+        if 'w16du' in NS:
+            marker.set(f"{{{NS['w16du']}}}dateUtc", date_iso)
+    rPr.insert(0, marker)
+    return para
+
+
...
+    rPr.insert(0, marker)
+
+    # Clear existing non-property content and rebuild inside <w:del>
...
+
+        if tag == 'equal':
+            # Only invoke paragraph diff when text or formatting differs
+            for (oe, ok), (ne, nk) in zip(old_slice, new_slice):
+                if ok == 'p' and nk == 'p':
+                    if _para_tokens_equivalent(oe, ne):
...
+
+    ns_map: dict[str, str] = {}
+    if 'word/document.xml' in new_parts:
+        ns_map = _register_namespaces_from_xml(new_parts['word/document.xml'])
+
+    # Parse documents
...
+
+    # Serialize
+    result_document_xml = serialize_xml(result_doc, extra_ns=ns_map)
+
+    # Enable track revisions
+    settings_xml = new_parts.get('word/settings.xml')
+    new_parts['word/settings.xml'] = _ensure_track_revisions(settings_xml)
+
