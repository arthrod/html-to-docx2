...
diff --git a/attempts/attempt_9_pro_new_styles.py b/attempts/attempt_9_pro_new_styles.py
new file mode 100644
index 0000000000000000000000000000000000000000..4d4221fb4eea1a3a287cd4169a7989ae50d2e200
--- /dev/null
+++ b/attempts/attempt_9_pro_new_styles.py
@@ -0,0 +1,562 @@
+#!/usr/bin/env python3
...
+    """Ensure settings.xml has <w:trackRevisions/> enabled and compliant prolog."""
+    if settings_bytes:
+        try:
...
+        return False
+
+    def _signature(elem: ET.Element) -> tuple:
+        return (
+            elem.tag,
+            tuple(sorted(elem.attrib.items())),
+            tuple(_signature(child) for child in elem),
+        )
+
...
+        return False
+    for old_tok, new_tok in zip(old_para.tokens, new_para.tokens):
+        if old_tok.text != new_tok.text:
+            return False
+        if not _same_rpr(old_tok.rPr, new_tok.rPr):
+            return False
+    return True
+
+
+def equal_runs_style(a_rPr: Optional[ET.Element], b_rPr: Optional[ET.Element]) -> bool:
+    return _same_rpr(a_rPr, b_rPr)
+
+
+def add_rPrChange(run_el: ET.Element, old_rPr: Optional[ET.Element], author: str, date: str, change_id: int) -> None:
+    """Append <w:rPrChange> capturing the prior run properties."""
+    rPr = run_el.find('w:rPr', NS)
+    if rPr is None:
+        rPr = ET.SubElement(run_el, W('rPr'))
+
+    ch = ET.Element(W('rPrChange'))
+    ch.set(W_ATTR('id'), str(change_id))
+    if author:
+        ch.set(W_ATTR('author'), author)
+    if date:
+        ch.set(W_ATTR('date'), date)
+
+    if old_rPr is not None:
+        ch.append(copy.deepcopy(old_rPr))
+    else:
+        ET.SubElement(ch, W('rPr'))
+
+    children = list(run_el)
+    if rPr in children:
+        insert_at = children.index(rPr) + 1
+        run_el.insert(insert_at, ch)
+    else:
+        run_el.append(ch)
+
+
...
+                if not equal_runs_style(tok_old.rPr, tok_new.rPr):
+                    add_rPrChange(run, tok_old.rPr, author, date, next_id)
+                    next_id += 1
+        elif tag == 'insert':
+            ins = _new_change_block(newp, 'ins', author, date, next_id)
+            next_id += 1
