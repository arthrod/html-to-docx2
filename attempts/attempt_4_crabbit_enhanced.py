...
def text_of_element(elem: etree._Element) -> str:
    ...
    """
    xpath_result = elem.xpath('.//w:t', namespaces=NSMAP)
    assert isinstance(xpath_result, list)
    # Filter to only _Element objects and extract text
    texts = []
    for item in xpath_result:
        if hasattr(item, 'text'):
            texts.append(item.text or '')  # type: ignore[union-attr]
    return ''.join(texts)
...
def extract_runs_from_hyperlink(hyperlink: etree._Element) -> list[etree._Element]:
    ...
    """
    result = hyperlink.xpath('./w:r', namespaces=NSMAP)
...
def _process_single_run(r: etree._Element, hyperlink: Optional[etree._Element]) -> list[dict[str, Any]]:
...
def wrap_run_in_hyperlink(r: etree._Element, hyperlink_template: etree._Element) -> etree._Element:
    ...
    for attr_name in [qn('r:id'), qn('w:anchor'), qn('w:tooltip')]:
        if attr_name in hyperlink_template.attrib:
            attr_value = hyperlink_template.get(attr_name)
...
def build_table_with_diffs(
    ...
    # Get all rows
    old_rows = old_tbl.xpath('./w:tr', namespaces=NSMAP)  # type: ignore[assignment]
    new_rows = result_tbl.xpath('./w:tr', namespaces=NSMAP)  # type: ignore[assignment]
...
def _diff_table_row(old_row: etree._Element, new_row: etree._Element,
    ...
    Args:
        ...
        cidgen: Change ID generator
    """
    old_cells = old_row.xpath('./w:tc', namespaces=NSMAP)  # type: ignore[assignment]
    new_cells = new_row.xpath('./w:tc', namespaces=NSMAP)  # type: ignore[assignment]
...
def _mark_row_as_inserted(row: etree._Element, author: str, date_iso: str, cidgen: ChangeIdGen) -> None:
    ...
    cells = row.xpath('./w:tc', namespaces=NSMAP)  # type: ignore[assignment]
...
def make_redline_docx(
    ...
    if old_body is None or new_body is None:
        raise ValueError("Document is missing w:body element")
